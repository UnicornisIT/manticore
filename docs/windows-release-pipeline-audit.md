...
6. Прочитать REST `/releases/{id}` и проверить digest, размер и официальный download URL installer для совместимости с updater. Это дополнительная проверка, а не замена вычислению хеша скачанных байтов. Digest в JSON-экспорте CLI не обязателен; отсутствие корректного digest в самом REST API блокирует публикацию, поскольку такой релиз отвергнет текущий клиент.
